...
param_grid = {'C': [10**(-5), 10**(-4), 10**(-3), 10**(-2), 10**(-1), 1, 10, 100, 1000]}
...
for train_index_outer, test_index_outer in outer_cv.split(x, y):
    ...
    selector = Sfs(estimator, k_features=1, cv=inner_cv, scoring='roc_auc', verbose=2,n_jobs=-2)
    ...
    grid_search = GridSearchCV(estimator, param_grid, cv=inner_cv,n_jobs=-2)
    grid_search.fit(x_train_selected, y_train_outer)
    beat_param = grid_search.best_params_
    print("最好的参数：", beat_param)
    y_pred = grid_search.predict(x_test_selected)
# y_test_outer对应的每个样本被预测为有问题的概率
    y_pred_pro = grid_search.predict_proba(x_test_selected)[:, 1]
